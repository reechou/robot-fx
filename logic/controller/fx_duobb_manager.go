...
func (daemon *Daemon) TaobaoGoodsSearchWithQuery(robot string, account *models.FxAccount) (*ext.GoodsSearchData, error) {
	robotAlimama := &models.FxRobotAlimama{
		RobotWX: robot,
	}
	has, err := models.GetRobotAlimama(robotAlimama)
	...
	pid := "mm_" + account.MemberId + "_" + account.GuideId + "_" + account.AdzoneId
	query := url.QueryEscape(account.LastSearchInfo)
	logrus.Debugf("goods search query: %s", query)
	searchReq := &ext.GoodsSearchReq{
		Alimama:  robotAlimama.Alimama,
		Pid:      pid,
		AdzoneId: account.AdzoneId,
	...
	logrus.Debugf("req: %v data: %v", searchReq, data)
	return data, nil
}
