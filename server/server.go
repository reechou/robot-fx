...
func (s *Server) serveAPI() error {
	...
	for _, srv := range s.servers {
		...
		go func(srv *HTTPServer) {
			var err error
			logrus.Infof("Server listen on %s", srv.l.Addr())
...
func (s *Server) makeHTTPHandler(handler utils.APIFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := handlerFunc(ctx, w, r, vars); err != nil {
			logrus.Errorf("Handler for %s %s returned error: %s", r.Method, r.URL.Path, errors.GetErrorMessage(err))
